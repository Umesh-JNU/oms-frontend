...
const Shop = () => {
  ...
  const getSubCate = async () => {
    ...
    try {
      const { data } = await axios.get(
        `/api/subCategory/${params?.id}/products`
      );
  ...
  const { data: shopDetailsData, isLoading: shopDetailsLoading } =
  ...
  useEffect(() => {
    ...
    if (window.location.href.includes("?sub=true")) {
      getSubCate();
    } else {
      getCategory();
    }
    ...
    setCategories(shopDetailsData?.categories);
  }, [params?.id, shopDetailsData]);
